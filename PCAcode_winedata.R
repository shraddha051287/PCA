...
# Considering top 3 principal component scores and binding them with mydat
win<-cbind(win,pcaObj$scores[,1:3])
View(win)
...
final1<-cbind(membership_1,win) # binding column wise with orginal data
View(final1)
View(aggregate(final1[,-c(1,16:18)],by=list(membership_1),FUN=mean)) # Inferences can be
...
normalized_data<-scale(win[,1:14]) 
...
k_5 <- kmeans(norm_clus,5)
win["Cluster"] <- k_5$clusters
aggregate(win[,-c(1,14)],by=list(win$cluster),mean)
